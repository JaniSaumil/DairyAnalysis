...
def load_model():
    try:
        # ✅ Reconstruct model from 6 parts if not already present
        if not os.path.exists('reconstructed_model.pkl'):
            required_parts = [f'model_part{i}.pkl' for i in range(1, 7)]
            if all(os.path.exists(part) for part in required_parts):
                with open('reconstructed_model.pkl', 'wb') as f_out:
                    for part in required_parts:
                        with open(part, 'rb') as f_in:
                            f_out.write(f_in.read())
                print("✅ Model reconstructed successfully as reconstructed_model.pkl")
            else:
                return None, "❌ Missing model part files. Ensure model_part1.pkl to model_part6.pkl are in the same folder."

        # ✅ Load reconstructed model
        model = joblib.load('reconstructed_model.pkl')
        return model, None
